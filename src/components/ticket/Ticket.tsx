...
const Ticket: React.FC<TicketProps> = ({ order }) => {
  ...
  const { id, eventName, startDate, startTime } = order;
  return (
    <button
      className="ticket"
      onClick={() => router.navigate({ to: `/details/${id}` })}
    >
      <div className="ticket__header">
        <span className="ticket__order-info">
          Ordine <span className="ticket__order-id">{id}</span> del {startDate}
        </span>
      </div>
      ...
      <div className="ticket__content">
        ...
        <PDFDownloadLink
          document={
            <TicketPDF biglietto={{ ...(user as any), ...(order as any) }} />
          }
