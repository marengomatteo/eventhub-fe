...
const FeedbackItem = ({ userName, comment }: { userName: string; comment: string }) => (
    ...
    useEffect(() => {
        ...
        const fetchEventData = async () => {
            try {
                ...
                const statsResponse = await getBaseURL("event").get(`/${eventId}/stats`);
                if (statsResponse.status === 200) {
                    setStats(statsResponse.data);
                }

                // Fetch feedback
                const feedbackResponse = await getBaseURL("feedback").get(`/event/${eventId}`);
