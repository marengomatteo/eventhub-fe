...
const EventDetailPage = () => {
  ...
  const fetchEventAgenda = useCallback(async (eventId: string) => {
    try {
      ...
      const response = await getBaseURL("agenda").get<AgendaResponse>(
        `/${eventId}`
      );
  ...
  if (error || !event) {
    return (
      <div className="event-detail-error">{error || "Evento non trovato"}</div>
    );
  }
                      ...
                      <div key={session.id} className="session-item">
                        ...
                        {session.speaker && (
                          <p className="session-speaker">
                            Relatore: {session.speaker.name}{" "}
                            {session.speaker.surname}
                          </p>
                        )}
...
const ConfirmationPage = () => {
  return (
    <div className="confirmation">
      ...
      <p>
        Il tuo biglietto è stato acquistato con successo, vai al tuo{" "}
        <a className="profile_link" href={"/profile"}>
          profilo
        </a>{" "}
        per vedere il biglietto.
      </p>
    </div>
  );
};
